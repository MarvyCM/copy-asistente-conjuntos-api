...
var metabuscador = {
    ...
    comportamiento: function(){
        var that = this;
        var localBody = this.body;
        var btn = this.header.find('.buscar a');
        var input = this.modalMetabuscador.find('.inputMetabuscador');
        var cerrar = this.modalMetabuscador.find('.icono.cerrar');

        btn.off('click').on('click', function(e){
            var button = $(this);
            if(that.modalMetabuscador.hasClass('busquedaActiva')){
                localBody.removeClass('metabuscadoron');
                that.modalMetabuscador.removeClass('busquedaActiva');
                button.find('.icono').removeClass('cerrar');
            } else {
                localBody.addClass('metabuscadoron');
                that.modalMetabuscador.addClass('busquedaActiva');
                input.focus();
                button.find('.icono').addClass('cerrar');
            }
        });
        ...
        cerrar.off('click').on('click', function(){
            localBody.removeClass('metabuscadoron');
            modalMetabuscador.removeClass('busquedaActiva');
            btn.find('.icono').removeClass('cerrar');
        });
        
        return;
    },
    ocultarResultados: function(){
        this.modalMetabuscador.removeClass('buscando');
    },
    mostrarResultados: function(){
        this.modalMetabuscador.addClass('buscando');
        this.comportamientoScrollHorizontal();
    },
    comportamientoScrollHorizontal: function() { 
        var children = this.resultadosMetabuscador.children();
        var metabuscadorWidth = this.resultadosMetabuscador.innerWidth();
        var childrenWidth = 0;
        children.each(function (index, element) {
            childrenWidth += $(this).innerWidth();
        });
        if(metabuscadorWidth < childrenWidth){
            this.anadirBoton();
        }
    },
    anadirBoton: function(){
        console.log(12)
    },
